...
def sort_rename_files(target_dir: str) -> None:
    """
    Given a file name, return a new file name with a random number appended to it.
    :param: target_dir: The original file name.
    :return: The new file name.
    """
    assert os.path.exists(target_dir), f"Directory {target_dir} does not exist."
    sorted_files = sorted(
        glob.glob(f"{target_dir}/*.wav"), key=os.path.getmtime)
    for i, f in enumerate(sorted_files, 1):
        os.rename(f, f"{target_dir}/{i}.wav")


def batch_wav_sample_rate_convert(
        source_dir: str,
        export_dir: str,
    ...
    assert os.path.exists(source_dir), f"Directory {source_dir} does not exist."
    ...
    for file in os.listdir(source_dir):
        ...
        if not clean_caches and os.path.exists(f"{export_dir}/{tail}"):
            print(f"File {tail} already exists. Skipping.")
        ret = subprocess.run([
            backends,
            abs_path,
            "-r",
